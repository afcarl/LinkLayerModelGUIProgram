...
function [topology, prrM, rssi] = LinkLayerModel(varargin)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Checking the validity of the two coefficients
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if (TEMPORAL_CORRELATION_COEFFICIENT<0 || SPATIAL_CORRELATION_COEFFICIENT<0)
    error('Correlation coefficients must be positive');
elseif (TEMPORAL_CORRELATION_COEFFICIENT>1 || SPATIAL_CORRELATION_COEFFICIENT>1)
    error('Correlation coefficients must be less than 1');
end

% declare two cells for the Gaussian fading variable
psi = cell(1,NUMBER_OF_ITERATIONS);
gfv = cell(1,NUMBER_OF_ITERATIONS);
...
rssi = cell(1,NUMBER_OF_ITERATIONS); %accordingly, our rssi should also be a cell
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pe = cell(1,NUMBER_OF_ITERATIONS);
for i=1:NUMBER_OF_NODES
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
prrM = cell(1,NUMBER_OF_ITERATIONS);
for i = 1:NUMBER_OF_NODES
    for j = 1:NUMBER_OF_NODES
        for t = 1:NUMBER_OF_ITERATIONS
            if (i == j)
...
% Overall result
link = zeros(NUMBER_OF_NODES^2,NUMBER_OF_ITERATIONS);
for t = 1:NUMBER_OF_ITERATIONS
    link(:,t) = reshape(prrM{1,t}',[],1);
end
linkimg = mat2gray(link);
figure(1)
imshow(linkimg,'XData',[1,NUMBER_OF_ITERATIONS],'YData',[1,NUMBER_OF_NODES^2])
title('Overall Result Based on the Parameters Entered');
xlabel('Number of Iteration (Temporal)'); ylabel('Link Number/Pair Number of Nodes (Spatial)');

% Create a text file with x-y coordinates for each node
foneID = fopen('xycoord.txt', 'w');
for i = 1:NUMBER_OF_NODES
    ix = floor(i/sqrt(NUMBER_OF_NODES))+1;
    iy = mod(i,sqrt(NUMBER_OF_NODES));
    if (iy==0)
        iy = sqrt(NUMBER_OF_NODES);
        ix = ix-1;
    end
    fprintf(foneID,'(%d,',ix);
    fprintf(foneID,'%d)\n',iy);
...
% Create a text file with PRRM value for each link
ftwoID = fopen('prrm.txt','w');
for t = 1:NUMBER_OF_ITERATIONS
    for i = 1:NUMBER_OF_NODES
        for j = 1:NUMBER_OF_NODES
           fprintf(ftwoID,'%d ',prrM{1,t}(i,j)); 
        end
        fprintf(ftwoID,'\n');
    end
    fprintf(ftwoID,'\n');
...
% Create a text file with RSSI value for each link
fthreeID = fopen('rssi.txt','w');
for t = 1:NUMBER_OF_ITERATIONS
    for i = 1:NUMBER_OF_NODES
        for j = 1:NUMBER_OF_NODES
           fprintf(fthreeID,'%d ',rssi{1,t}(i,j)); 
        end
        fprintf(fthreeID,'\n');
    end
    fprintf(fthreeID,'\n');
